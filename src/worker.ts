...
import type { ComparePagespeedInput, RunPagespeedInput } from "../src/core/pagespeed.js";
...
function asToolResult(payload: unknown): ToolResult {
  return {
...
function parseUrl(params: Record<string, unknown>): string {
...
function toRunPagespeedInput(input: unknown): RunPagespeedInput {
...
function toComparePagespeedInput(input: unknown): ComparePagespeedInput {
  const params = asObject(input);
  return {
    url: parseUrl(params),
    categories: parseStringArray(params.categories),
...
function toVersionCheckInput(input: unknown): { force_refresh?: boolean; timeout_ms?: number } {
  const params = asObject(input);
  return {
